...
public interface ReleaserEventRepository extends JpaRepository<ReleaserEvent, Long>, JpaSpecificationExecutor {
    ...
    @Query(value = "select * from releaser_event as e where e.start_time is not null and e.end_time is not null order by e.start_time desc limit 1 offset 1", nativeQuery = true)
    public ReleaserEvent findPenultimateCompletedEvent();
